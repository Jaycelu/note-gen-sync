...
class SyslogConfigurator:
    ...
    def __init__(self, csv_file, loghost='10.40.29.201', max_workers=10, update_callback=None):
        ...
        self.device_type_mapping = {
            'huawei': 'huawei',
            'h3c': 'hp_comware',
            'hp_comware': 'hp_comware',  # 添加对hp_comware的直接支持
            'ruijie': 'ruijie_os',
            'cisco': 'cisco_ios',
            'cisco_ios': 'cisco_ios',
            'cisco_xe': 'cisco_xe',
    ...
    def generate_config_commands(self, host_ip, device_type):
        ...
        elif device_type in ['h3c', 'hp_comware']:
            return [
                f"info-center loghost {self.loghost}",
                "info-center enable",
                "info-center console channel 6",
                "info-center console log level informational"
            ]
    ...
    def verify_config(self, conn, host_ip, device_type):
        ...
        try:
            if device_type == 'huawei':
                ...
                lines = output.split('\n')
                for line in lines:
                    if self.loghost in line:
                        self.logger.info(f"找到包含日志服务器地址的行: {line.strip()}")
                        return True
            elif device_type in ['h3c', 'hp_comware']:
                output = conn.send_command("display info-center")
                self.logger.info(f"验证输出: {output}")
    ...
    def configure_device(self, row):
        ...
        try:
            ...
            with ConnectHandler(**device_config) as conn:
                ...
                conn.enable()
                config_commands = self.generate_config_commands(host_ip, ssh_type)
                ...
                if self.verify_config(conn, host_ip, ssh_type):
    ...
    def run_configuration(self):
        ...
        summary = self.print_summary(start_time)
        return summary
...
class SyslogApp:
    ...
    def export_results(self):
        ...
        if file_path and self.results_summary:
            try:
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(str(self.results_summary))
    ...
    def run_config(self):
        ...
        try:
            ...
            if self.results_summary:
                self.result_text.insert(tk.END, str(self.results_summary), "summary")
            self.result_text.config(state="disabled")
